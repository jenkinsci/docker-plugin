...
public class DockerContainerWatchdog extends AsyncPeriodicWork {
     ...
     */
    private static final long RECURRENCE_PERIOD_IN_MS = JenkinsUtils.getSystemPropertyLong(
                    DockerContainerWatchdog.class.getName() + ".recurrenceInSeconds", 5L * 60L)
            * 1000L;
    ...
    protected boolean stopAndRemoveContainer(
            ...
            String containerId,
            boolean stop) {
        return DockerTransientNode.stopAndRemoveContainer(
                dockerApi, aLogger, description, removeVolumes, containerId, stop);
    }
    ...
    protected void execute(TaskListener listener) throws IOException, InterruptedException {
        ...
        try {
            ...
            try {
                ...
                LOGGER.warn(
                        "Processing of cleanup watchdog took too long; current timeout value: {}, "
                                + "watchdog started on {}",
                        PROCESSING_TIMEOUT,
                        start.toString(),
                        timeout);
                executionStatistics.addProcessingTimeout();
                executionStatistics.addOverallRuntime(
    ...
    private Map<String, Node> loadNodeMap() {
        ...
        LOGGER.debug(
                "We currently have {} nodes assigned to this Jenkins instance, which we will check", nodeMap.size());

        return nodeMap;
    }

    private ContainerNodeNameMap processCloud(
    ...
    private ContainerNodeNameMap retrieveContainers(DockerCloud dc, DockerClient client)
            throws ContainersRetrievalException {
        /*
         * Note:
        ...
        try {
            ...
            try {
                ...
                LOGGER.warn(
                        "Unable to retrieve list of containers available on DockerCloud [name={}, dockerURI={}] while reading list of containers (showAll=true, labelFilters={})",
                        dc.getDisplayName(),
                        dc.getDockerApi().getDockerHost().getUri(),
                        labelFilter.toString(),
                        e);
            ...
            for (Container container : containerList) {
                ...
                if (status == null) {
                    LOGGER.warn(
                            "Container {} has a null-status and thus cannot be checked; ignoring container",
                            containerId);
                    continue;
                }
                ...
                if (containerNodeName == null) {
                    LOGGER.warn(
                            "Container {} is said to be created by this Jenkins instance, but does not have any node name label assigned; manual cleanup is required for this",
                            containerId);
                    continue;
                }
            ...
            Instant stop = clock.instant();
            executionStatistics.addRetrieveContainerRuntime(
    ...
    private void cleanUpSuperfluousContainers(
            DockerClient client,
            Map<String, Node> nodeMap,
            ContainerNodeNameMap csm,
            DockerCloud dc,
        ...
        for (Container container : allContainers) {
            ...
            LOGGER.info(
                    "Container {}, which is reported to be assigned to node {}, "
    ...
    private static boolean isStillTooYoung(Long created, Instant snapshotInstant) {
         ...
         */
        final long graceDurationForContainersInSeconds = JenkinsUtils.getSystemPropertyLong(
                DockerContainerWatchdog.class.getName() + ".initialGraceDurationForContainersInSeconds", 60L);
    ...
    private void terminateContainer(DockerCloud dc, DockerClient client, Container container) {
        ...
        if (gracefulFailed) {
            try {
                ...
                Instant stop = clock.instant();
                executionStatistics.addContainerRemovalForce(
                        Duration.between(start, stop).toMillis());
    ...
    private void terminateContainerGracefully(DockerCloud dc, Container container)
            throws TerminationException, ContainerIsTaintedException {
        String containerId = container.getId();
        ...
        if (nodeExistsBypassingCache(nodeName)) {
            LOGGER.warn(
                    "Was going to terminate container ID {}, but a node for it has appeared so it does not need removing now.",
                    container.getId());
            throw new ContainerIsTaintedException(
                    String.format("Node for container ID %s has appeared", container.getId()));
        }

        String removeVolumesString = containerLabels.get(DockerContainerLabelKeys.REMOVE_VOLUMES);
        if (removeVolumesString == null) {
            throw new ContainerIsTaintedException(String.format(
                    "Container ID %s has no '%s' label; skipping.",
        ...
        DockerAPI dockerApi = dc.getDockerApi();
        Instant start = clock.instant();
        boolean success = stopAndRemoveContainer(
                dockerApi,
                LOGGER,
                String.format("(orphaned container found by %s)", DockerContainerWatchdog.class.getSimpleName()),
                removeVolumes,
                container.getId(),
                !containerRunning);
        Instant stop = clock.instant();
        ...
        if (success) {
            executionStatistics.addContainerRemovalGracefully(
                    Duration.between(start, stop).toMillis());
    ...
    private void cleanUpSuperfluousComputer(
            Map<String, Node> nodeMap, ContainerNodeNameMap csmMerged, Instant snapshotInstant) {
        for (Node node : nodeMap.values()) {
            ...
            LOGGER.info(
                    "{} has container ID {}, but the container does not exist in any docker cloud. Will remove node.",
                    dtn,
                    dtn.getContainerId());
