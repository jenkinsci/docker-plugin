...
class DockerDelegatingComputerLauncher extends DelegatingComputerLauncher {
	...
	private class DockerLog implements ResultCallback<Frame> {
		...
		public void onNext(Frame object) {
			LOGGER.log(LOG_LEVEL,
					"Container " + containerId + " logged: " + object.toString());
		}
	...
	public void beforeDisconnect(SlaveComputer computer, TaskListener listener) {
		...
		if (shouldSeeWhatTheContainerLogged) {
			...
			try (final DockerClient client = api.getClient()) {
				client.logContainerCmd(containerId).withStdErr(true).withStdOut(true).withTail(100).exec(callback);
			} catch (Exception ignored) {
				LOGGER.log(LOG_LEVEL,
						"Container " + containerId + " log could not be inspected.", ignored);
			}
		}
